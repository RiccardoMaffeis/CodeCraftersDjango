document.addEventListener('DOMContentLoaded', function () {
  ...
  ui.timeSlots.forEach(slot => {
    slot.addEventListener('click', () => {
      ...
      fetch(
        `/biglietti/get_proiezione_id/?film=${filmId}&data=${date}&ora=${oraCompleta}`
      )
        .then(res => {
        ...
        .then(data => {
          ...
          fetch('/biglietti/salva_parametri/', {
            method: 'POST',
            headers: {
  ...
  function generateSeatsFromSala (salaId) {
    fetch('/biglietti/get_sala_data/?id=' + salaId)
      .then(res => res.json())
      .then(data => {
  ...
  function createSeatElement (seatNumber, occupiedSeats, riga, numRighe) {
    ...
    if (!occupied) {
      seat.addEventListener('click', () =>
        toggleSeatSelection(seat, seatNumber, type.price)
      )
    }
    return seat
  }
  ...
  function updateCart () {
    if (bookingState.selectedSeats.length === 0) {
      ...
      if (manualDiscount > 0 && code !== 'CINEMA20' && code !== 'LOGIN10') {
    ...
    ui.seatsList.innerHTML = bookingState.selectedSeats
      .map(s => `<li>Posto ${s.number} (€${s.price})</li>`)
      .join('')
    const discounted =
      bookingState.totalPrice * (1 - bookingState.discountApplied / 100)
  ...
  function getOccupiedSeats () {
    ...
    return fetch(
      `/biglietti/get_proiezione_id/?film=${filmId}&data=${date}&ora=${ora}`
    )
      .then(r => r.json())
      .then(d => {
        const proiezioneId = d.proiezioneId
        if (!proiezioneId) return []
        return fetch(
          `/biglietti/get_posti_occupati/?proiezione=${proiezioneId}`
        ).then(r => r.json())
      })
      .catch(err => {
        console.error(err)
  ...
  ui.checkoutBtn.addEventListener('click', () => {
    if (!bookingState.selectedTime || bookingState.selectedSeats.length === 0) {
      showCustomAlert(
        'warning',
        'Completa tutti i passaggi!',
        'Seleziona orario e posti'
      )
      return
    }
    const filmTitle =
      document.querySelector('.movie-title span')?.textContent.trim() || ''
    const conferma =
      `📽️ Film: ${filmTitle}<br>⏰ Orario: ${bookingState.selectedTime}` +
      `<br>💺 Posti: ${bookingState.selectedSeats
        .map(s => s.number)
        .join(', ')}` +
      `<br>💰 Totale: €${(
        bookingState.totalPrice *
        (1 - bookingState.discountApplied / 100)
      ).toFixed(2)}`
  ...
  function salvaBigliettiEInvia (email, filmTitle, proiezioneId) {
    fetch('/biglietti/salva_biglietti/', {
      method: 'POST',
      ...
      .then(resp => {
        if (resp.status === 'ok') {
          ...
          showCustomAlert(
            'error',
            'Errore durante il salvataggio dei biglietti'
          )
        }
      })
      .catch(err => {
  ...
  function inviaMail (email, film) {
    ...
    fetch('/biglietti/send_ticket_email/', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'X-CSRFToken': csrftoken },
